...
parser.add_argument("--schedule", default="Seg", type=str)
...
writer = SummaryWriter(comment=' FedDF with local epoch 5')
...
def run_experiments(experiments):
    ...
    for xp_count, xp in enumerate(experiments):
        ...
        client_loaders, train_loader, test_loader, stats = data_slover.get_data_loaders(hp)
        ...
        for c_round in range(1, hp['communication_rounds'] + 1):
            ...
            if xp.is_log_round(c_round):
                ...
                results_test = server.evaluate(iter=c_round)

                writer.add_scalar('Eva/Loss', results_test['loss'], c_round * hp['local_iterations'])
                writer.add_scalar('Eva/Accuracy', results_test['accuracy'], c_round * hp['local_iterations'])
                writer.add_scalar('Eva/mean IoU', results_test['mean IoU'], c_round * hp['local_iterations'])
